...
ConfigBuilder.prototype.register = function(params) {
	...
	if (!name) {
		throw new ConfigError('Can\'t register config with empty name');
	}

	if (name in this.configs) {
		throw new ConfigError('Config with name "' + name + '" already exists');
	}

	var parentName = params.parent;

	if (parentName && !(parentName in this.configs)) {
	...
	var parentConfig = parentName ? this.configs[parentName] : {};

	var parentChain = parentName ? parentConfig.__info__.chain : [];

	// add __info__ field to config
	var __info__ = {
		name: name,
		parentName: parentName,
		chain: parentChain.concat(name)
	};

	this.configs[name] = extend({}, parentConfig, config, {__info__: __info__});
};
...
ConfigBuilder.prototype.update = function(params) {
	...
	if (!name) {
		throw new ConfigError('Can\'t update config with empty name');
	}
...
ConfigBuilder.prototype.get = function(name) {
	if (!name) {
		throw new ConfigError('Can\'t get config with empty name');
	}
